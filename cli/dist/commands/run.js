...
async function runPlatform(platform, options) {
    const spinner = (0, ora_1.default)(`Running ${platform} app...`).start();
    try {
...
async function startWebServer(webPath, port) {
    // Check if node_modules exists
    const nodeModulesPath = path.join(webPath, 'node_modules');
    if (!(await (0, fs_1.pathExists)(nodeModulesPath))) {
        logger_1.logger.info('Installing web dependencies...');
        const installResult = await (0, exec_1.executeCommand)('npm', ['install'], {
            cwd: webPath,
            stdio: 'inherit'
        });
    ...
    // Start dev server
    webServerProcess = (0, child_process_1.spawn)('npm', ['run', 'dev'], {
        cwd: webPath,
        stdio: 'pipe',
        shell: true,
        detached: false
...
async function updateAndroidServerUrl(androidPath, serverUrl) {
    ...
    if (mainActivityFile) {
        let content = await (0, fs_1.readFile)(mainActivityFile);
        // Replace serverUrl in loadGyoConfig function
        content = content.replace(/serverUrl = "http:\/\/[^"]+"/, `serverUrl = "${serverUrl}"`);
        await (0, fs_1.writeFile)(mainActivityFile, content);
    }
}
async function updateIOSServerUrl(iosPath, serverUrl) {
    ...
    await (0, fs_1.writeFile)(configPath, JSON.stringify(config, null, 2));
}
async function runAndroid(device, spinner, serverUrl) {
    const androidPath = path.join(process.cwd(), 'android');
    if (!(await (0, fs_1.pathExists)(androidPath))) {
        spinner.fail('Android project not found');
        logger_1.logger.error('Build the Android project first: gyo build android');
        process.exit(1);
    }
    // Check if adb is available
    if (!(await (0, exec_1.checkCommandExists)('adb'))) {
        spinner.fail('adb not found');
        logger_1.logger.error('Please install Android SDK and add adb to your PATH');
        process.exit(1);
    }
    ...
    const devicesResult = await (0, exec_1.executeCommand)('adb', ['devices'], { stdio: 'pipe' });
    if (!devicesResult.success || !devicesResult.stdout.includes('device')) {
        spinner.fail('No Android devices found');
        logger_1.logger.error('Connect a device or start an emulator');
        process.exit(1);
    }
    ...
    if (!selectedDevice) {
        ...
        if (deviceLines.length > 0) {
            ...
            if (deviceLines.length > 1) {
                logger_1.logger.info(`Multiple devices found. Using: ${selectedDevice}`);
            }
        }
    }
    // Build and install
    ...
    const buildResult = await (0, exec_1.executeCommand)(gradlew, ['assembleDebug'], {
        cwd: androidPath,
        stdio: 'pipe'
    });
    ...
    const installResult = await (0, exec_1.executeCommand)(gradlew, ['installDebug'], {
        cwd: androidPath,
        stdio: 'inherit'
    });
    if (!installResult.success) {
    ...
    if (packageName && selectedDevice) {
        ...
        const launchResult = await (0, exec_1.executeCommand)('adb', launchArgs, { stdio: 'pipe' });
        if (launchResult.success) {
            spinner.succeed('App installed and launched on Android device!');
        }
        else {
            spinner.succeed('App installed on Android device!');
            logger_1.logger.warn('Could not auto-launch app. Please launch manually.');
        }
    }
    else {
        spinner.succeed('App installed on Android device!');
    ...
    logger_1.logger.log('');
    logger_1.logger.success(`App is connected to: ${serverUrl}`);
    logger_1.logger.info('Monitoring console logs (Press Ctrl+C to stop)...');
    logger_1.logger.log('');
    ...
    const logcat = (0, child_process_1.spawn)('adb', logcatArgs, {
...
async function runIOS(device, spinner, serverUrl) {
    const iosPath = path.join(process.cwd(), 'ios');
    if (!(await (0, fs_1.pathExists)(iosPath))) {
        spinner.fail('iOS project not found');
        logger_1.logger.error('Build the iOS project first: gyo build ios');
        process.exit(1);
    }
    ...
    const buildResult = await (0, exec_1.executeCommand)('xtool', ['dev'], {
        cwd: iosPath,
        stdio: 'pipe'
    });
    if (!buildResult.success) {
        spinner.fail('Build failed');
        logger_1.logger.error(buildResult.stderr || 'Unknown error');
        process.exit(1);
    }
    ...
    if (fullBundleId === bundleId && bundleId && await (0, exec_1.checkCommandExists)('idevicesyslog')) {
        spinner.text = 'Looking for installed app...';
        const syslogCapture = (0, child_process_1.spawn)('idevicesyslog', [], {
            stdio: ['ignore', 'pipe', 'pipe']
        });
        const foundBundleId = await new Promise((resolve) => {
            let timeout;
            let found = false;
            const safeBundleId = bundleId || '';
            syslogCapture.stdout?.on('data', (data) => {
    ...
    if (fullBundleId !== bundleId) {
        logger_1.logger.info(`Found full bundle ID: ${fullBundleId}`);
    }
    else {
        logger_1.logger.info(`Using bundle ID: ${bundleId}`);
    }
    // App installed successfully
    spinner.succeed('App installed on iOS device!');
    bundleId = fullBundleId;
    logger_1.logger.log('');
    logger_1.logger.info('📱 Please tap the app icon on your device to launch it.');
    logger_1.logger.log('');
    logger_1.logger.log('');
    logger_1.logger.success(`App is connected to: ${serverUrl}`);
    logger_1.logger.info('Monitoring console logs (Press Ctrl+C to stop)...');
    ...
    if (await (0, exec_1.checkCommandExists)('idevicesyslog')) {
        const syslog = (0, child_process_1.spawn)('idevicesyslog', ['-m', bundleId], {
            stdio: ['ignore', 'pipe', 'pipe']
        });
        ...
        syslog.stderr?.on('data', (data) => {
            // Ignore stderr for now
        });
    }
    else {
        logger_1.logger.warn('idevicesyslog not found. Install libimobiledevice for log monitoring.');
    }
    // Keep process running
    await new Promise(() => { });
}
async function runDesktop(spinner, serverUrl) {
    ...
    if (!(await (0, fs_1.pathExists)(desktopPath))) {
        ...
        logger_1.logger.error('Build the desktop project first: gyo build desktop');
        process.exit(1);
    }
    spinner.text = 'Starting desktop app...';
    const runResult = await (0, exec_1.executeCommand)('npm', ['start'], {
        cwd: desktopPath,
        stdio: 'inherit'
    });
...
async function getAndroidPackageName(androidPath) {
    try {
        const buildGradlePath = path.join(androidPath, 'app/build.gradle');
        if (!(await (0, fs_1.pathExists)(buildGradlePath))) {
            return null;
        }
...
async function getIOSBundleId(iosPath) {
    try {
        const xtoolYmlPath = path.join(iosPath, 'xtool.yml');
        if (!(await (0, fs_1.pathExists)(xtoolYmlPath))) {
            return null;
        }
        const content = await (0, fs_1.readFile)(xtoolYmlPath);
